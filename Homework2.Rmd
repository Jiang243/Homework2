...
```
We should not use rings to predict age because we use rings to caculate age. When we use rings to predict age, our R^2 will becomes 1 even we do not include other predictors in our model. Therefore, our model is not meanful when we include rings.
...
abalone_metrics(abalone_train_res, truth = age, 
                estimate = .pred)
```
R^2 is 0.5633, which means 56.33% of the variability in Y(age) that can be explained by using predictor variables
